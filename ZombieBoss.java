...
public class ZombieBoss extends Movable{
	...
	int attackRectangleSize = 300; //Range of the attack
	
	public ZombieBoss(Handler handler, float x, float y) {
		...
		bounds.width = 185;
		bounds.height = 100;
		health = 20;//20 health for the zombie boss
	}
	...
	private void getMovement(){
		...
		if(time % (random.nextInt(50) + 30) == 0){
			xMove = random.nextInt(3) - 1;
			yMove = random.nextInt(3) - 1;
			
			//Add or subtract more if the random number is 1 or -1 to add more speed
			if(xMove == 1){
				xMove = 3; //Temporary zombie speed for x
			}
			if(xMove == -1){
				xMove = -3; //Temporary zombie speed for x
			}
			if(yMove == 1){
				yMove = 3; //Temporary zombie speed for y
			}
			if(yMove == -1){
				yMove = -3; //Temporary zombie speed for y
			}
	...
	public void die(){
		...
		int random = rand.nextInt(150) + 1; //Better chance to get better stuff from the boss
		...
		handler.getWorld().setDeathNum(-2);
		handler.getWorld().setZombieSpawn(1);
		handler.getWorld().setWinCheck(1);
	...
	//Getting the animation from the zombie
	  private BufferedImage getCurrentAnimationFrameZombie(){
		  	if(xMove < 0){
				return Assets.zombieBossLeft;
			}
			else if (xMove > 0){
				return Assets.zombieBossRight;
			}
			else if (yMove < 0){
				return Assets.zombieBossUp;
			}
			else{
				return Assets.zombieBossDown;
			}
	  }
